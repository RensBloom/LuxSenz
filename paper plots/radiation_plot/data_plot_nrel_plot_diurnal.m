...
plot([time_data(1),time_data(end)],[i2 i2],'DisplayName','>50m');
...
plot([1,N_per_day],[i2 i2],'DisplayName','>50m');
% legend('Location','Best');
...
%     S{s} = r;
    mins(s,:) = min(r')';
    maxs(s,:) = max(r')';
    handles = plot(time_r,r,'Color',[1 1 1]*0.7,'LineWidth',0.02);
...
plot([time_r(1) time_r(end)],[irradiance irradiance],'Color',colors(5,:),'DisplayName','Power','LineWidth',2);
