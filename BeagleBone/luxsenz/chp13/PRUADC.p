...
// Constants from the MCP3201 datasheet 
#define TIME_CLOCK      50       // T_hi and t_lo  (25 = 250ns (2MHz), 50 = 1 MHz) = 50 instructions (min)
...
START:
	...
	MOV	r7, 0x00001FFF	 // the bit mask to use on the returned data (i.e., keep 12 LSBs only)
	LBBO    r8, r1, 4, 4     // load the Linux address that is passed into r8 -- to store sample values
	LBBO	r10,r1, 4, 4     // and into r10
...
SPICLK_BIT:                      // loop for each of the 16 bits
	...
	CALL	SPICLK           // repeat call the SPICLK procedure until all 17-bits written/read
	QBNE	SPICLK_BIT, r4, 0

	SET	r30.t5		 // pull the CS line high (end of sample)
	LSR	r3, r3, 2        // SPICLK shifts left too many times left, shift right once (MCP3x02/8) or twice (MXP3201)
...
STORE_DATA:                      // store the sample value in memory
	...
	SBBO	r8, r10, 0, 4    // store current address location into start of memory block
	ADD	r8, r8, 2	 // shifting by 2 bytes - 2 bytes per sample
